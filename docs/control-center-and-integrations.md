...
Known defect in the v1 worker contracts as they stand on `main`: `worker-result.request` requires a `job_id` that the v1 contract never supplies, and `worker-poll.response.job` has no declared shape. A fix is **Not implemented yet (tracked in [#221](https://github.com/ahliweb/omes/issues/221))** — proposed in unmerged PR `ahliweb/omes#222`. Consumers must not assume the published v1 worker schemas are internally consistent until that lands.
...
- Prototype redesign v2 driven by v1 contract fixtures — **Not implemented yet (tracked in [#211](https://github.com/ahliweb/omes/issues/211))**, proposed in unmerged PR `ahliweb/omes#212`.
- v1 worker contract fix (`worker-result.request` `job_id`, `worker-poll.response.job` shape) — **Not implemented yet (tracked in [#221](https://github.com/ahliweb/omes/issues/221))**, proposed in unmerged PR `ahliweb/omes#222`.
- AI data-privacy boundary, machine-readable egress policy, restricted local-only inference posture, privacy-posture evidence and projection, and the matching regression coverage — **Not implemented yet (tracked in [#213](https://github.com/ahliweb/omes/issues/213), [#214](https://github.com/ahliweb/omes/issues/214), [#215](https://github.com/ahliweb/omes/issues/215), [#216](https://github.com/ahliweb/omes/issues/216), [#217](https://github.com/ahliweb/omes/issues/217), [#218](https://github.com/ahliweb/omes/issues/218))**, proposed in the unmerged stack `ahliweb/omes#219`, `#223`, `#224`, `#225`, `#226`, `#227`. This repository has no AI data-classification or model-egress policy today.
- A dedicated enrollment-token management screen (`omes_control.enrollments.manage`) — not implemented, and no owning OMES issue exists for it.
