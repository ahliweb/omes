...
> Status: evidence record, compiled 2026-09-24. This document states what
...
> parent [#195](https://github.com/ahliweb/omes/issues/195) (epic).
> Neither issue is closed by this document; closing them is a maintainer
> decision once the open OMES pull requests recorded in §6 are merged.
...
## 6. Deferred work — not shipped

Each item below exists only as an open OMES issue and, where noted, an
unmerged pull request. Nothing here is available on `main`.

| Capability | State |
|---|---|
| Control Center prototype redesign v2 driven by v1 contract fixtures | Not implemented yet (tracked in [#211](https://github.com/ahliweb/omes/issues/211)); proposed in unmerged PR `ahliweb/omes#212`. `ui/control-center/index.html` on `main` is still the v1 prototype adopted under ADR-0023. |
| `worker-result.request` requiring a `job_id` the v1 contract never provides, and `worker-poll.response.job` having no shape | Not implemented yet (tracked in [#221](https://github.com/ahliweb/omes/issues/221)); proposed in unmerged PR `ahliweb/omes#222`. The v1 worker contracts on `main` still carry the defect. |
| AI data-privacy boundary and local/cloud model policy | Not implemented yet (tracked in [#213](https://github.com/ahliweb/omes/issues/213)); proposed in unmerged PR `ahliweb/omes#219`. |
| Machine-readable AI data-classification and egress policy | Not implemented yet (tracked in [#214](https://github.com/ahliweb/omes/issues/214)); proposed in unmerged PR `ahliweb/omes#223`. |
| AI privacy posture and egress evidence without raw prompt capture | Not implemented yet (tracked in [#216](https://github.com/ahliweb/omes/issues/216)); proposed in unmerged PR `ahliweb/omes#224`. |
| Control Center projection of AI privacy posture and policy decisions | Not implemented yet (tracked in [#217](https://github.com/ahliweb/omes/issues/217)); proposed in unmerged PR `ahliweb/omes#225`. |
| Restricted local-only inference deployment posture for Hermes | Not implemented yet (tracked in [#215](https://github.com/ahliweb/omes/issues/215)); proposed in unmerged PR `ahliweb/omes#226`. |
| AI privacy boundary regression and exfiltration-resistance coverage | Not implemented yet (tracked in [#218](https://github.com/ahliweb/omes/issues/218)); proposed in unmerged PR `ahliweb/omes#227`. |
...
PRs `#223`, `#224`, `#225`, `#226` and `#227` are a linear stack whose base is
PR `#219`, whose base is `main`. None of them has merged.
...
A broader AI data-egress policy is **not implemented yet (tracked in
[#213](https://github.com/ahliweb/omes/issues/213)–[#218](https://github.com/ahliweb/omes/issues/218))**.
Until that work merges, this repository has no machine-readable
data-classification or model-egress policy, and the close-out makes no claim
that one exists.
...
`VERSION` on `main` is `0.3.0`; released tags are `v0.1.0`, `v0.2.0`,
`v0.3.0`. Versioning is a post-merge step: `scripts/release.sh` consumes the
accumulated `changes/*.md` fragments and must run on a clean, green `main`
after the pull requests in §6 land, per
[ADR-0010](adr/0010-versioning-and-change-fragments.md).

The next release should therefore cover, at minimum, the fragments already
accumulated on `main` since `v0.3.0` — including the Hermes/Graphify/content
upstream-delegation refactors, the release provenance and SBOM work, the
Ubuntu 26.04 and Linux Mint 22.3 compatibility additions, the Control Center
UI/UX design system (#200), the pull-worker transport (#192), the Hermes
orchestration visualization (#183), and this close-out (#202). Because those
fragments include new user-visible capabilities rather than only fixes, a
minor bump to `0.4.0` is the expected shape. The exact number, and whether any
of the §6 pull requests have landed by then, is the release cutter's call at
the time of the cut — this document does not pre-commit it.
