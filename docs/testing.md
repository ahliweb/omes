...
Every fixture value in this suite is synthetic. The only secret-*shaped* literal is the
`sk_live_CANARY_...` canary already allowlisted in `.gitleaks.toml` by a CANARY-pinned regex
(added by #216); this suite added no new gitleaks allowlist entry and contains no real, revoked,
or ever-valid credential. See [docs/ci.md](ci.md) section 7 for the procedure that would apply
if a new entry were ever genuinely needed.
