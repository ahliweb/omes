...
Every fixture value here is synthetic. The only secret-SHAPED literal is
the `sk_live_CANARY_...` canary already allowlisted in `.gitleaks.toml`
by a CANARY-pinned regex (added by #216); this file adds no new gitleaks
allowlist entry and contains no real, revoked, or ever-valid credential.
"""
import json
import os
import shutil
...
#: `jobs.schema.scan_for_raw_secrets` rejects, and the CANARY-pinned
#: `.gitleaks.toml` allowlist regex, so it can be committed while still
#: being rejected by the real secret gate.
CANARY_API_KEY = "sk_live_CANARY_218RegressionOnly"  # gitleaks:allow
